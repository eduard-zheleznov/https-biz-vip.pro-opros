...
function buildSurveyAnalysisContext(input: AnalyzeSurveyResultInput) {
  return [
    "Если ответ помечен как \"Не отвечено (начислен средний балл)\", это техническая заглушка старого подсчета, а не содержательный ответ кандидата. Для AI-оценки пригодности считай такой пункт отсутствием фактов и красным флагом.",
    "Если анкета в основном не заполнена, содержит только контакты, короткие общие фразы или большинство ключевых вопросов помечены как неотвеченные, итоговая категория должна быть КРАСНЫЙ независимо от технически начисленных баллов.",
    "Не повышай категорию до ЖЕЛТЫЙ только из-за начисленного среднего балла; желтая зона возможна только при нескольких содержательных ответах с проверяемыми фактами.",
  ...
  return [
    {
      ...
      content: [
        ...
        "Пропущенные ответы с пометкой \"Не отвечено (начислен средний балл)\" считай отсутствием ответа; такие технические баллы не должны поднимать почти пустую анкету выше красной зоны.",
